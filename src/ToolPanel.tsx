...
const ToolPanel: React.FC = () => {
  ...
  // adjustUnread 内部 useCallback([]) 稳定，store 收到同一引用即可。
  ...
  // 统一订阅 IDLE 新邮件事件（preload 单订阅覆盖式，故由本组件集中管理）：
  //   1. scheduleUnreadRefresh → 未读计数去抖刷新
  //   2. 按当前视图触发列表刷新（统一视图聚合 / 单账号单视图）
  useEffect(() => {
  ...
  return (
    <div className="one-email-plugin-root flex h-full min-h-0 w-full bg-background text-foreground">
      <Sidebar
        accounts={accounts}
      ...
      <MailList
        ...
        onSearch={(q) => (isUnified ? unifiedStore.runSearch(q) : mailStore.runSearch(q))}
